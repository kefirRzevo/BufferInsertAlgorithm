...
void Config::read(std::istream &Is) {
  ...
  Tech = Technology{
      .UnitWireCapacitance = UnitWireRFloat.template get<Technology::FloatTy>(),
      .UnitWireCapacitance = UnitWireCFloat.template get<Technology::FloatTy>(),
      .UnitWireCapacitanceComment =
          UnitWireRCommentStr.template get<std::string>(),
      .UnitWireResistanceComment =
